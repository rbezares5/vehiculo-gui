...
class Vehicle:
    def __init__(self):
        ...
        self.labelframe1=tk.LabelFrame(self.ventana1, text='Engine')
        self.labelframe1.grid(column=0, row=1, sticky='we')

        self.labelEngine=tk.Label(self.labelframe1, text=str(self.engine))
        self.labelEngine.pack()

        # añadir para widget scale luminosidad
        self.labelframe2=tk.LabelFrame(self.ventana1, text='Environment Light')
        self.labelframe2.grid(column=0, row=2, sticky='we')

        self.lumScale=tk.Scale(self.labelframe2, from_=0, to=100, orient=HORIZONTAL, length=1000, command=self.update_lum)
        self.lumScale.pack()
        self.lumScale.set(self.environment.get_lum())


        # añadir barra progreso combustible
        self.labelframe3=tk.LabelFrame(self.ventana1, text='Fuel Level')
        self.labelframe3.grid(column=0, row=3, sticky='we')

        self.fuelBar=ttk.Progressbar(self.labelframe3, orient=HORIZONTAL, length=1000)
        self.fuelBar.pack()
        self.fuelBar['value']=self.fuel.get_porcentage_level()
    ...
    def do_work(self):
        ...
        # actualizamos valores de los widgets
        self.labelEngine.configure(text=str(self.engine))
        self.lumScale.set(self.environment.get_lum())
        self.fuelBar['value']=self.fuel.get_porcentage_level()
    
    def update_lum(self,event):
        self.environment.set_lum(self.lumScale.get())
